...
contract ownable{
    ...
    modifier yoShit(){//This modifier checks to see if the current user is the owner; an error will be thrown if the current user is not the owner of the contract
        require(msg.sender == owner, "Nah nigga, this ain't even yo shit fr");//This is the error message that will be thrown
        _;
    }
    function genWit() external view yoShit{//This function can only be called by the contract owner
    ...
    function newOwnership(address newOwner) external yoShit{//This function can only be called by the contract owner
